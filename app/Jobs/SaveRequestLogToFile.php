...
#[Timeout(30)]
#[Backoff(5, 15, 30)]
class SaveRequestLogToFile implements ShouldQueue, ShouldBeEncrypted
{
    use Dispatchable;
    ...
    /**
     * Prevent concurrent writes for the same request.
     */
    public function middleware(): array
    {
        $key = (string)($this->metadata['request_id'] ?? Str::uuid());

        return [
            (new WithoutOverlapping("request-log:{$key}"))
                ->releaseAfter(5)
                ->expireAfter(60),
        ];
    }
    ...
    public function handle(): void
    {
        ...
        if (!$isEmpty) {
            ...
            if ($rawBody !== '' && $rawBody !== false) {
                $rawPath = "raw/{$dirPrefix}/{$baseName}.{$storedExtension}";
    ...
    private function saveResponseBody(Filesystem $disk, string $dirPrefix, string $baseName): ?string
    {
        if (!$this->shouldSaveResponse()) {
            return null;
        }
        ...
        $path = "response/{$dirPrefix}/{$baseName}.txt";
        $disk->put($path, $body);
     ...
     */
    private function saveMetadata(Filesystem $disk, string $dirPrefix, string $baseName, array $metadata): void
    {
        try {
            $json = json_encode(
                $metadata,
            ...
            $disk->put("metadata/{$dirPrefix}/{$baseName}.json", $json);
