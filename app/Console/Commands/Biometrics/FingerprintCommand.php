...
class FingerprintCommand extends Command
{
    ...
    public function handle(DeviceCommandQueue $queue): int
    {
        ...
        $body = match ($action) {
            ...
            'enroll' => $pin
                ...
                : $this->abortWithError('--pin required for enroll'),
            'query' => $pin
                ? CommandBuilder::queryFingerprint($pin, $fid)
                : $this->abortWithError('--pin required for query'),
            default => $this->abortWithError('action must be update, delete, enroll, or query'),
        };
    ...
    private function buildUpdate(mixed $pin, ?int $fid): ?string
    {
        if (! $pin || $fid === null || ! $this->option('template')) {
            return $this->abortWithError('--pin, --fid and --template are required for update');
        }
