...
class PhotoCommand extends Command
{
    ...
    public function handle(DeviceCommandQueue $queue): int
    {
        ...
        $body = match ($action) {
            ...
            'delete-user' => $pin
                ...
                : $this->abortWithError('--pin required for delete-user'),
            'update-bio' => $this->buildBioUpdate($pin),
            'delete-bio' => $pin
                ? CommandBuilder::deleteBioPhoto($pin)
                : $this->abortWithError('--pin required for delete-bio'),
            default => $this->abortWithError('action must be update-user, delete-user, update-bio, or delete-bio'),
        };
    ...
    private function buildUserUpdate(mixed $pin): ?string
    {
        if (! $pin || ! $this->option('content')) {
            return $this->abortWithError('--pin and --content are required for update-user');
        }
    ...
    {
        if (! $pin || ! $this->option('content')) {
            return $this->abortWithError('--pin and --content are required for update-bio');
        }
