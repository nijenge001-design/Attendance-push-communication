...
class DeviceDataHandler
{
    ...
    public function handleAttendanceLogs(string $sn, string $content): string
    {
        ...
        foreach ($this->iterateLines($content) as $line) {
            ...
            try {
                ...
                DispatchMailJob::push(new ProcessAttendanceLog($log));
    ...
    /**
     * OPERLOG — persist USER lines in this request so attendees exist even
     * when RabbitMQ/Octane drops the job. Bios still go through the job
    ...
    public function handleOperationLogs(string $sn, string $content): string
    {
        ...
        $result = $this->processOperLogContent($sn, $content);

        Log::channel('user_info')->info('OPERLOG processed in-request', [
            'sn' => $sn,
    ...
    public function handleUserInfo(string $sn, string $content): string
    {
        ...
        foreach ($this->iterateLines($content) as $line) {
            if (! str_starts_with($line, 'USER') && ! str_contains($line, 'PIN=') && ! str_contains($line, 'Pin=')) {
                continue;
            }
            $this->parseAndStoreUser($sn, $line);
            $count++;
    ...
    private function parseAndStoreFingerprint(string $sn, string $line): void
    {
        ...
        DispatchMailJob::push(new ProcessBioTemplate($template));
    }
    ...
    private function parseAndStoreBioPhoto(string $sn, string $line): void
    {
        ...
        DispatchMailJob::push(new StoreBioPhotoJob($sn, $pin, $filename, $type, $base64));
    }
