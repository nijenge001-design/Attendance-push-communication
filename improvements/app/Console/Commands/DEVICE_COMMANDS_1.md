...
These map 1:1 to `CommandBuilder` categories. Target selection:

| Flag | Meaning |
|------|---------|
| `--serial=SN` | One device |
| `--status=approved` | All devices with that status (default when no serial) |
| `--all` | Every device |
| `--force` | Skip multi-device confirmation |

### User management — `device:user`

```bash
php artisan device:user update --serial=CPV2232560484 --pin=1001 --name="Jane Doe" --card=12345
php artisan device:user delete --serial=CPV2232560484 --pin=1001
php artisan device:user query --serial=CPV2232560484
php artisan device:user query --serial=CPV2232560484 --pin=1001
php artisan device:user update --status=approved --pin=1001 --name="Jane" --force
```

### Fingerprint — `device:fp`

```bash
php artisan device:fp enroll --serial=CPV2232560484 --pin=1001 --fid=0
php artisan device:fp update --serial=CPV2232560484 --pin=1001 --fid=0 --template=BASE64...
php artisan device:fp query --serial=CPV2232560484 --pin=1001
php artisan device:fp delete --serial=CPV2232560484 --pin=1001 --fid=0
php artisan device:fp delete --serial=CPV2232560484 --pin=1001
```

### Face — `device:face`

```bash
php artisan device:face update --serial=CPV2232560484 --pin=1001 --template=BASE64...
php artisan device:face query --serial=CPV2232560484 --pin=1001
php artisan device:face delete --serial=CPV2232560484 --pin=1001
```

### Finger vein — `device:fv`

```bash
php artisan device:fv update --serial=CPV2232560484 --pin=1001 --fid=0 --template=BASE64...
php artisan device:fv delete --serial=CPV2232560484 --pin=1001 --fid=0
```

### Unified biometrics (BIODATA) — `device:bio`

```bash
php artisan device:bio update --serial=CPV2232560484 --pin=1001 --type=9 --index=0 --template=BASE64...
php artisan device:bio query --serial=CPV2232560484 --pin=1001 --type=9
php artisan device:bio delete --serial=CPV2232560484 --pin=1001 --type=9
```

### Photos — `device:photo`

```bash
php artisan device:photo update-user --serial=CPV2232560484 --pin=1001 --content=BASE64...
php artisan device:photo delete-user --serial=CPV2232560484 --pin=1001
php artisan device:photo update-bio --serial=CPV2232560484 --pin=1001 --content=BASE64...
php artisan device:photo delete-bio --serial=CPV2232560484 --pin=1001
```

### Attendance logs — `device:attlog`

```bash
php artisan device:attlog query --serial=CPV2232560484 --start="2026-09-01 00:00:00" --end="2026-09-07 23:59:59"
php artisan device:attlog upload --status=approved --force
php artisan device:attlog clear --serial=CPV2232560484
```

### System — `device:sys`

```bash
php artisan device:sys info --serial=CPV2232560484
php artisan device:sys check --status=approved --force
php artisan device:sys set-time --status=approved --force
php artisan device:sys set-time --datetime="2026-09-07 12:00:00" --serial=CPV2232560484
php artisan device:sys set-option --serial=CPV2232560484 --option=Door1=1 --option=Lock=1
php artisan device:sys unlock --serial=CPV2232560484
php artisan device:sys reboot --serial=CPV2232560484 --force
php artisan device:sys clear-data --serial=CPV2232560484 --force
php artisan device:sys shell --serial=CPV2232560484 --cmd="ls /mnt"
```

### File operations — `device:file`

```bash
php artisan device:file put --serial=CPV2232560484 --path=/path/on/device.txt --content=...
php artisan device:file get --serial=CPV2232560484 --path=/path/on/device.txt
php artisan device:file delete --serial=CPV2232560484 --path=/path/on/device.txt
```

### Publicity pictures — `device:ad`

```bash
php artisan device:ad update --serial=CPV2232560484 --filename=ad1.jpg --content=BASE64...
php artisan device:ad query --serial=CPV2232560484
php artisan device:ad delete --serial=CPV2232560484 --filename=ad1.jpg
```
...
| Command | Category |
|---------|----------|
| `device:user` | User Management |
| `device:fp` | Biometrics (Fingerprint) |
| `device:face` | Biometrics (Face) |
| `device:fv` | Biometrics (Finger Vein) |
| `device:bio` | Biometrics (Unified) |
| `device:photo` | Biometrics (Photos) |
| `device:attlog` | Attendance Logs |
| `device:sys` | System Commands |
| `device:file` | File Operations |
